...
// Map of board square indices to chess notation
...
// Check if there’s insufficient material to continue the game
...
void pushMove(Board* board, const Move& move) {
    ...
    if (isEnPassantMove) { makeEnPassantMove(board, move); return; }
    if (move.castle) { makeCastleMove(board, move); return; }
